...
def resize_and_padding(image, size, scale_factor=0.75):
    # Padding to size ratio
    w, h = image.size
    target_w, target_h = int(size[0] * scale_factor), int(size[1] * scale_factor)
    if w / h < target_w / target_h:
        new_h = target_h
        new_w = w * target_h // h
    else:
    ...
    padding.paste(image, ((size[0] - new_w) // 2, (size[1] - new_h) // 2))
    return padding
