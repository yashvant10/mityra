...
@router.get("/")
async def get_personalized_recommendations(user = Depends(get_current_user)):
    try:
        user_id = user["uid"]
        recommendations = await firebase_service.get_recommendations(user_id)
        ...
        return {"recommendations": recommendations, "source": "personalized"}
